...
# Not hightly correlated @ 0.5 - 1-5, 37, 46, 56, 64-67, 89, 144
...
mult_result <- multinom(ds_new_status_bmd$Status_BMD ~
                          Age + BMI + Smoking + Alcohol + Total.C + LDLsize + Unsaturation + His + Acetate +
                            Acetoacetate + Acetone + Albumin + L.VLDL.TG + XL.HDL.FC,
...
mult_result <- multinom(ds_new_status_bmd$Status_BMD ~
                          Age + BMI  + His + Acetoacetate + Acetone + XL.HDL.FC,
                        maxit = 1000, data = ds_new_status_bmd)
summary(mult_result)
...
lm_result <- glm(Status_Fractures ~
                   Age + BMI + Smoking + Alcohol + Total.C + LDLsize + Unsaturation + His + Acetate +
                     Acetoacetate + Acetone + Albumin + L.VLDL.TG + XL.HDL.FC,
...
lm_result <- glm(Status_Fractures ~
                   Age + Total.C + LDLsize + His + Acetate + Acetone + Albumin + L.VLDL.TG + XL.HDL.FC,
                 family = 'binomial', data = ds_new_status_fractures)
summary(lm_result)
...
## coefficients (STD err has to be less than half of model co-off) and tab model
mult_result <- multinom(LS_status ~
                          Age + BMI + LDLsize + His + Acetone + L.VLDL.TG + XL.HDL.FC,
                        maxit = 1000, data = ds_new_ls_bmd)
summary(mult_result)
tab_model(mult_result, show.est=FALSE)
...
mult_result <- multinom(FN_status ~
                          Age + BMI + LDLsize +
                            Acetoacetate + Acetone + XL.HDL.FC,
                        maxit = 1000, data = ds_new_fn_bmd)
summary(mult_result)
tab_model(mult_result, show.est=FALSE)
...
## coefficients (STD err has to be less than half of model co-off) and tab model
mult_result <- multinom(TH_status ~
                          Age + BMI + LDLsize + His + Acetone + XL.HDL.FC,
                        maxit = 1000, data = ds_new_th_bmd)
summary(mult_result)
tab_model(mult_result, show.est=FALSE)
